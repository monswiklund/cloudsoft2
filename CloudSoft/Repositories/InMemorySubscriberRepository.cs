...
public class InMemorySubscriberRepository : ISubscriberRepository
{
    ...
    public Task<bool> AddAsync(Subscriber subscriber)
    {
        ...
        // TryAdd returns true if the key is added, false if it already exists
    ...
    public Task<bool> UpdateAsync(Subscriber subscriber)
    {
        ...
        // We need to hadnle the update manually since we need to check if the key already exists
    ...
    public Task<bool> DeleteAsync(string email)
    {
        ...
        // TryRemove returns true if the key is removed, false if it doesn't exist
        return Task.FromResult(_subscribers.TryRemove(email, out _));
    }

public Task<bool> ExistsAsync(Subscriber subscriber)
{
    if (subscriber == null || string.IsNullOrEmpty(subscriber.Email))
    {
        return Task.FromResult(false);
    ...
    return Task.FromResult(_subscribers.ContainsKey(subscriber.Email));
}
}